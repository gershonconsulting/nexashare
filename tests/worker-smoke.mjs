...
assert.match(dashboardSource, /EXPECTED_EXTENSION_VERSION\s*=\s*'1\.2\.8'/);
...
assert.match(dashboardSource, /nexashare-extension-1\.2\.8\.zip/);
...
assert.match(extensionSetupSource, /Current version 1\.2\.8/);
assert.match(extensionSetupSource, /nexashare-extension-1\.2\.8\.zip/);
...
assert.match(extensionSource, /could not identify LinkedIn\\'s visible direct Repost choice/);
