...
assert.match(workerSource, /%\[0-9A-Fa-f\]\{2\}/);
...
assert.match(extensionSource, /REPOST_ATTEMPTS = 4/);
assert.match(extensionSource, /linkedin-layout:fingerprint/);
assert.match(extensionSource, /repost:progressive-retry/);
assert.match(extensionSource, /structural-card/);
assert.match(extensionSource, /activity-link/);
assert.match(extensionSource, /semantic-control/);
assert.doesNotMatch(extensionSource, /CIRCUIT_BREAKER_THRESHOLD/);
assert.doesNotMatch(extensionSource, /linkedin-discovery:circuit-open/);
assert.match(extensionSource, /republier\|republication\|reposter\|reposten\|teilen/);
assert.match(extensionSource, /outcomeId: crypto\.randomUUID\(\)/);
...
assert.match(workerSource, /recordDeliveryOutcome/);
assert.match(workerSource, /\/api\/campaign-health/);
assert.match(workerSource, /\/api\/collections/);
...
assert.match(dashboardSource, /Sources monitored/);
assert.match(dashboardSource, /Repost attempts/);
...
assert.match(extensionSource, /repost\|reshare\) instantly/);
assert.match(extensionSource, /with your thoughts\|quote/);
assert.match(extensionSource, /view repost\|view reshare/);
assert.match(extensionSource, /republier/);
assert.match(extensionSource, /republication/);
assert.match(extensionSource, /reposten/);
assert.match(extensionSource, /repost rückgängig/);
assert.match(extensionSource, /voir la republication/);
...
assert.match(extensionSource, /waitForLinkedInPosts/);
assert.match(extensionSource, /POST_DISCOVERY_ATTEMPTS/);
